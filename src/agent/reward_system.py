...
#
# BUG FIXED:
#   The old version used ANOMALY THRESHOLDS only, and the anomaly score it
#   thresholded was itself wrong: the LSTM autoencoder was never loaded, so
#   "anomaly" was the reconstruction error of a RANDOMLY INITIALISED network,
#   scored on a single unscaled reading. Under that broken feature the class
#   ordering was inverted (Perfume appeared most anomalous, Mixture least), so
#   the old reward said "Monitor" is correct for Mixture — backwards, since
#   Mixture needs Emergency Shutdown.
#
# CORRECTED FACT (do not propagate the old claim):
#   With the pretrained AE loaded and the full 20-step window scored
#   (retrain/raw_pipeline.py, src/tools/anomaly_tool.py), mean reconstruction
#   error TRACKS hazard as expected:
#       NoGas 0.74  <  Perfume 1.5  <<  Mixture 118  <  Smoke 229
#   Earlier drafts of this file and of README.md asserted "Mixture has anomaly
#   ~0.10, the lowest of all gases". That is FALSE and was an artefact of the
#   untrained autoencoder. It must not be quoted anywhere.
#
# FIX:
#   Use gas_id + action, identical to the training notebook. Ground-truth class
#   is the correct reward signal here; the anomaly-only path below is retained
#   for blind live mode and is documented as unreliable.
#
...
def compute_reward(
    ...
    if gas_id is not None:
        ...
        # WARNING: the anomaly-only path is a blind-mode fallback. Even with the
        # corrected AE (anomaly now tracks hazard: NoGas 0.74 < Perfume 1.5 <<
        # Mixture 118 < Smoke 229) the thresholds below were tuned against the
        # OLD broken feature and have not been recalibrated. Do not evaluate on it.
...
def _reward_anomaly_only(anomaly: float, action: int) -> float:
    ...
    WARNING: the fixed thresholds below (0.20 / 0.50 / 0.80 / 0.90) were tuned
    against the OLD, broken anomaly feature produced by an untrained
    autoencoder, whose class ordering was inverted. With the corrected feature
    the raw reconstruction error spans NoGas 0.74 -> Smoke 229, so these
    normalized cut-points no longer correspond to anything meaningful.
    Do NOT use this path for evaluation or training until it is recalibrated.
    """
    if anomaly < 0.20:
