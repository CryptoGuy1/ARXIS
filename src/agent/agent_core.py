...
""".strip()
    ...
    def run_once(
        ...
        # Audit fix #2: feed the FULL rolling 20-step window to the anomaly tool
        # (matching retrain/raw_pipeline — window-level scaled AE reconstruction
        # error, not a single unscaled reading).
        anomaly_score_raw = float(self.anomaly.compute_from_window(sensor_window))
        ...
        action_after_safety = int(safety_override(
            state.tolist(),
            int(action_raw),
            gas_id=gas_id,
            raw_anomaly=anomaly_score_raw,
        ))
