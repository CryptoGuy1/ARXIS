...
`verify_v13.py` answers "does every number match the result files". This answers
...
def prose_only(doc):
    ...
    for ln in doc.split("\n"):
        ...
        if not st or st.startswith(("|", "![", "#", ">", "&nbsp;")):
            continue
        if re.match(r"^\*\*(?:Table|Fig)\.? ?[0-9A-Z]", st):
            continue
...
SUPERSEDED = [
    ("factor of 40", None, "the held-out-class bound ratio is about 86 after the worst-partition correction"),
    ...
    ("Table 15", None, "the paper has fourteen tables"),
...
CONCL = section(TXT, "## Conclusion", ["## Nomenclature"])
RQTABLE = TXT[:TXT.index("**Table 6—Research questions")].rstrip().split("\n\n")[-1]
BODY = TXT[TXT.index("## Experimental Results and Discussion"):]
...
COHERENCE = [
    ("the miss-bound ratio", "about 86", [SUMMARY, RQTABLE, CONCL, BODY]),
