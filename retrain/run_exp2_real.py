...
because the v1 report under retrain/results/ cites it. It does not participate
in the verification run that checks every reported value against a stored
result file, and its header below describes the decision network in
reinforcement-learning terms although it is trained by cost-weighted
cross-entropy and always was. Use the driver named in Table A-1 for the
quantity you want.
...
# was a hard-coded path to one author's machine; made repository-relative so
# the file is at least honest about where it expects to run. It remains
# deprecated: see the notice above.
