...
![](figures_v2/fig_perception.png){width=6.50in}
...
![](figures_v2/fig_costsweep.png){width=6.50in}
...
![](figures_v2/fig_calibration.png){width=6.50in}

Fig. S3—Calibration. (a) ECE with bootstrap intervals. (b) The same on a logarithmic axis, split into all windows against hazardous windows only. (c) Sensitivity to the number of bins. MC dropout uses 20 passes and the deep ensemble five members.
...
![](figures_v2/fig_ablation.png){width=6.50in}
...
![](figures_v2/fig_anominfluence.png){width=6.50in}
...
Experiment 8 reports the alarm-mapped case on the held-out Mixture class. Both rejection policies on both held-out classes are here (Table S9, `v3_reject_option.csv`), because the choice between them is the result. All rows are fitted on the reduced partition that remains once the calibration band is carved out, so the closed-set values they reduce to at *q* = 0 are the within-experiment reference rather than the Table 10 entries.
...
Experiment 9 reports the worst value across comparators. The per-model breakdown is here (Table S10, `v3_raw_faults.csv`), as missed-hazard rate followed by escalation adequacy.
...
### S10 Anomaly Detector in Detail
...
![](figures_v2/fig_roc.png){width=6.50in}
